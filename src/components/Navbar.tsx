...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* MOBILE SIDE DRAWER MENU (Side Drawer / Sliding Panel) */}
      <div
      ...
      >
        ...
        {/* Side Panel (Sliding in from Right Side) */}
        <div
          className={`fixed top-0 right-0 bottom-0 w-[84%] max-w-xs sm:max-w-sm bg-[#FFFDF0] shadow-2xl border-l-2 border-amber-300 flex flex-col justify-between transition-transform duration-300 ease-in-out z-10 ${
            mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        >
          {/* Top Bar inside Side Drawer */}
          <div className="p-5 border-b border-amber-200/80 bg-white/80 flex items-center justify-between">
          ...
          <div className="overflow-y-auto p-5 space-y-5 flex-1">
            {/* Language Selector In Mobile Side Drawer */}
            ...
            <nav className="space-y-1.5">
              ...
              <button
                onClick={() => handleLinkClick('#services')}
                className="w-full py-3 px-3.5 rounded-2xl font-black uppercase tracking-wider text-xs text-slate-800 hover:bg-amber-100 hover:text-black transition-all text-left flex items-center justify-between cursor-pointer"
              >
              ...
              <button
                onClick={() => handleLinkClick('#track')}
                className="w-full py-3 px-3.5 rounded-2xl font-black uppercase tracking-wider text-xs text-amber-950 bg-[#FED74C]/35 hover:bg-[#FED74C]/60 border border-amber-300 transition-all text-left flex items-center justify-between cursor-pointer shadow-2xs"
              >
                <div className="flex items-center gap-2">
            ...
            {/* Quick Dispatch Hotline info badge */}
            <div className="p-3.5 rounded-2xl bg-white border border-amber-200/80 space-y-1.5">
              ...
              <p className="text-[11px] text-slate-600 flex items-center gap-1.5">
                ...
                <span>Direct Hotline: 0917-555-8812</span>
              </p>
            </div>
          </div>

          {/* Bottom Actions: Dispatch Portal & Order Now Button */}
          <div className="p-5 border-t border-amber-200/80 bg-white/90 space-y-2.5">
            <button
              onClick={() => {
                setMobileMenuOpen(false);
                onOpenAdmin();
