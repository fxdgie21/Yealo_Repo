...
import { OrderRecord, Product, ProductBagOption, DeliveryRider } from '../types';
...
export const AdminDispatchModal: React.FC<AdminDispatchModalProps> = ({
  ...
  // Delivery Fleet & Riders state
  const [riders, setRiders] = useState<DeliveryRider[]>(() => {
    try {
      const stored = localStorage.getItem('yealo_saved_riders');
      if (stored) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed.map((p, idx) => ({
            ...
            phone: p.phone || '',
            vehicle: p.vehicle || 'Insulated Ice Box Tricycle',
    ...
    return [
      { id: 'rider-arnel', name: 'Kuya Arnel', phone: '09625415941', vehicle: 'Insulated Ice Box Tricycle', plateNumber: 'TR-3121', status: 'Available', notes: 'Muñoz & San Jose City Route', createdAt: new Date().toISOString() },
      { id: 'rider-jun', name: 'Kuya Jun', phone: '0917-555-8812', vehicle: 'Motorcycle with Cold Chest', plateNumber: 'MC-8812', status: 'Available', notes: 'Express deliveries & downtown', createdAt: new Date().toISOString() },
      { id: 'rider-mark', name: 'Kuya Mark', phone: '0998-333-2211', vehicle: 'Express Multicab / Delivery Van', plateNumber: 'VN-2211', status: 'Available', notes: 'Bulk restaurant & commercial orders', createdAt: new Date().toISOString() },
    ];
  });
  const savedRiders = riders;
  ...
  const [riderFormVehicle, setRiderFormVehicle] = useState('Insulated Ice Box Tricycle');
  const [riderFormPlate, setRiderFormPlate] = useState('');
  const [riderFormStatus, setRiderFormStatus] = useState<'Available' | 'On Delivery' | 'Off Duty'>('Available');
  const [riderFormNotes, setRiderFormNotes] = useState('');
  const [riderSearchQuery, setRiderSearchQuery] = useState('');
  const [riderStatusFilter, setRiderStatusFilter] = useState<string>('all');
  const [assignToOrderRider, setAssignToOrderRider] = useState<DeliveryRider | null>(null);

  // Rider assignment states for specific orders
  const [orderForRiderAssign, setOrderForRiderAssign] = useState<OrderRecord | null>(null);
  const [assignRiderName, setAssignRiderName] = useState('');
  const [assignRiderPhone, setAssignRiderPhone] = useState('');
  const [assignRiderNotes, setAssignRiderNotes] = useState('');
  ...
  const [mCity, setMCity] = useState('San Jose, Philippines, 3121');
  ...
  const [mRiderName, setMRiderName] = useState('');
  const [mRiderPhone, setMRiderPhone] = useState('');
  ...
  // Real-time synchronization for Delivery Fleet & Riders
  useEffect(() => {
    if (!isOpen || !isAuthenticated) return;
        ...
        if (!snapshot.empty) {
          ...
          snapshot.forEach((d) => {
            ...
            list.push({
              ...
              phone: data.phone || '',
              vehicle: data.vehicle || 'Insulated Ice Box Tricycle',
          ...
          setRiders(list);
          try {
            localStorage.setItem('yealo_saved_riders', JSON.stringify(list));
          } catch {}
        } else {
          // If Firestore collection is empty, seed defaults
          const defaultList: DeliveryRider[] = [
            { id: 'rider-arnel', name: 'Kuya Arnel', phone: '09625415941', vehicle: 'Insulated Ice Box Tricycle', plateNumber: 'TR-3121', status: 'Available', notes: 'Muñoz & San Jose City Route', createdAt: new Date().toISOString() },
            { id: 'rider-jun', name: 'Kuya Jun', phone: '0917-555-8812', vehicle: 'Motorcycle with Cold Chest', plateNumber: 'MC-8812', status: 'Available', notes: 'Express deliveries & downtown', createdAt: new Date().toISOString() },
            { id: 'rider-mark', name: 'Kuya Mark', phone: '0998-333-2211', vehicle: 'Express Multicab / Delivery Van', plateNumber: 'VN-2211', status: 'Available', notes: 'Bulk restaurant & commercial orders', createdAt: new Date().toISOString() },
          ];
          defaultList.forEach(async (r) => {
            try {
              await setDoc(doc(db, 'riders', r.id), r);
            } catch {}
          });
          setRiders(defaultList);
        }
      },
      (err) => {
        console.warn('Riders listener error, falling back to local cache:', err);
      }
    );
    return () => unsubRiders();
  }, [isOpen, isAuthenticated]);
  ...
  const handleOpenRiderAssign = (order: OrderRecord) => {
    setOrderForRiderAssign(order);
    setAssignRiderName(order.riderName || '');
    setAssignRiderPhone(order.riderPhone || '');
    setAssignRiderNotes(order.riderNotes || '');
  };

  const handleSaveRider = async (andDispatch = false) => {
    if (!orderForRiderAssign) return;
    if (!assignRiderName.trim()) {
      alert('Please enter rider full name');
      return;
    }

    setIsSavingRider(true);
    const orderId = orderForRiderAssign.id;
    const newStatus = andDispatch ? ('Out for Delivery' as const) : orderForRiderAssign.status;

    const riderData = {
      riderName: assignRiderName.trim(),
      riderPhone: assignRiderPhone.trim(),
      riderNotes: assignRiderNotes.trim(),
      status: newStatus,
      updatedAt: serverTimestamp(),
    };

    try {
      await updateDoc(doc(db, 'orders', orderId), riderData);
      ...
      // If active slip matches, update it too
      ...
      // Save rider to directory for quick future re-use
      if (assignRiderName.trim()) {
        const found = riders.find((r) => r.name.toLowerCase() === assignRiderName.trim().toLowerCase());
        const riderId = found ? found.id : `rider-${Date.now()}`;
        const riderDoc: DeliveryRider = {
          id: riderId,
          name: assignRiderName.trim(),
          phone: assignRiderPhone.trim() || found?.phone || '',
          vehicle: assignRiderNotes.trim() || found?.vehicle || 'Insulated Ice Box Tricycle',
          plateNumber: found?.plateNumber || '',
          status: andDispatch ? 'On Delivery' : (found?.status || 'Available'),
          notes: found?.notes || '',
          createdAt: found?.createdAt || new Date().toISOString(),
        };
        try {
          await setDoc(doc(db, 'riders', riderId), riderDoc);
        } catch {}
        setRiders((prev) => {
          const exists = prev.some((r) => r.id === riderId);
          if (exists) {
            return prev.map((r) => (r.id === riderId ? riderDoc : r));
          }
          return [...prev, riderDoc];
        });
      }

      setActionMessage(
        andDispatch
          ? `Rider "${assignRiderName.trim()}" assigned & marked Out for Delivery!`
          : `Rider details for Order #${orderForRiderAssign.orderNumber} saved!`
      );
      setOrderForRiderAssign(null);
      setTimeout(() => setActionMessage(null), 3500);
    } catch (err) {
      console.warn('Firestore rider update notice:', err);
      setOrders((prev) =>
        prev.map((o) =>
          o.id === orderId
            ? {
      ...
      setOrderForRiderAssign(null);
      setActionMessage(`Rider details updated.`);
      setTimeout(() => setActionMessage(null), 3000);
    } finally {
  ...
  // Fleet Rider Management Handlers
  const handleOpenAddRider = () => {
    setEditingRider(null);
    setRiderFormName('');
    setRiderFormPhone('');
    setRiderFormVehicle('Insulated Ice Box Tricycle');
    setRiderFormPlate('');
    setRiderFormStatus('Available');
    setRiderFormNotes('');
    setShowAddRiderModal(true);
  };

  const handleOpenEditRider = (rider: DeliveryRider) => {
    setEditingRider(rider);
    setRiderFormName(rider.name);
    setRiderFormPhone(rider.phone);
    setRiderFormVehicle(rider.vehicle || 'Insulated Ice Box Tricycle');
    setRiderFormPlate(rider.plateNumber || '');
    setRiderFormStatus(rider.status || 'Available');
    setRiderFormNotes(rider.notes || '');
    setShowAddRiderModal(true);
  };

  const handleSaveRiderForm = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!riderFormName.trim()) {
      alert('Please enter Rider full name');
      return;
    }
    if (!riderFormPhone.trim()) {
      alert('Please enter Rider contact mobile phone number');
      return;
    }

    const riderId = editingRider ? editingRider.id : `rider-${Date.now()}`;
    const payload: DeliveryRider = {
      id: riderId,
      name: riderFormName.trim(),
      phone: riderFormPhone.trim(),
      vehicle: riderFormVehicle.trim() || 'Insulated Ice Box Tricycle',
      plateNumber: riderFormPlate.trim() || undefined,
      status: riderFormStatus,
      notes: riderFormNotes.trim() || undefined,
      createdAt: editingRider?.createdAt || new Date().toISOString(),
    };

    try {
      await setDoc(doc(db, 'riders', riderId), payload);
      setRiders((prev) => {
        const idx = prev.findIndex((r) => r.id === riderId);
        if (idx >= 0) {
          const next = [...prev];
          next[idx] = payload;
          return next;
      ...
      try {
        const next = editingRider
          ? riders.map((r) => (r.id === riderId ? payload : r))
          : [...riders, payload];
        localStorage.setItem('yealo_saved_riders', JSON.stringify(next));
      } catch {}

      setActionMessage(
        editingRider
          ? `Updated rider "${payload.name}" details!`
          : `Added new rider "${payload.name}" (${payload.phone}) to fleet directory!`
      );
      setShowAddRiderModal(false);
      setEditingRider(null);
      setTimeout(() => setActionMessage(null), 3500);
    } catch (err) {
      console.warn('Failed to save rider to Firestore:', err);
      setRiders((prev) => {
        const filtered = prev.filter((r) => r.id !== riderId);
  ...
  const handleDeleteRider = async (rider: DeliveryRider) => {
    if (!confirm(`Are you sure you want to remove rider "${rider.name}" (${rider.phone}) from fleet directory?`)) {
      return;
    }
    try {
      await deleteDoc(doc(db, 'riders', rider.id));
    } catch (err) {
      console.warn('Failed to delete rider from Firestore:', err);
    }
    setRiders((prev) => prev.filter((r) => r.id !== rider.id));
    try {
      const next = riders.filter((r) => r.id !== rider.id);
      localStorage.setItem('yealo_saved_riders', JSON.stringify(next));
    } catch {}
    setActionMessage(`Rider "${rider.name}" removed from fleet.`);
    setTimeout(() => setActionMessage(null), 3000);
  };

  const handleToggleRiderStatus = async (rider: DeliveryRider, newStatus: DeliveryRider['status']) => {
    try {
      await updateDoc(doc(db, 'riders', rider.id), { status: newStatus });
    } catch (err) {
  ...
  const handleAssignRiderToOrderQuick = async (rider: DeliveryRider, order: OrderRecord) => {
    const orderId = order.id;
    const riderData = {
      riderName: rider.name,
      riderPhone: rider.phone,
      riderNotes: rider.vehicle || '',
      status: 'Out for Delivery' as const,
      updatedAt: serverTimestamp(),
    };
    try {
      await updateDoc(doc(db, 'orders', orderId), riderData);
      await updateDoc(doc(db, 'riders', rider.id), { status: 'On Delivery' });
    } catch {}
    setOrders((prev) =>
      prev.map((o) =>
        o.id === orderId
    ...
    setRiders((prev) =>
      prev.map((r) => (r.id === rider.id ? { ...r, status: 'On Delivery' } : r))
    );
    setAssignToOrderRider(null);
    setActionMessage(`Order #${order.orderNumber} assigned to ${rider.name} (${rider.phone})!`);
    setTimeout(() => setActionMessage(null), 3500);
  };
  ...
  const handleCreateManualOrder = async (e: React.FormEvent) => {
    ...
    const newOrd: OrderRecord = {
      ...
      riderName: mRiderName.trim() || undefined,
      riderPhone: mRiderPhone.trim() || undefined,
      status: mRiderName.trim() ? 'Out for Delivery' : 'Pending',
    };
  ...
  const modalContent = (
    ...
    >
      ...
      >
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50/60">
            ...
            {showManualOrderForm && (
              ...
              >
                ...
                {/* Rider Assignment Fields for Manual Order */}
                <div className="p-3 rounded-xl bg-amber-100/70 border border-amber-300 space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="block text-slate-800 text-xs font-black flex items-center gap-1.5">
                      ...
                      <span>Assign Delivery Rider (Optional)</span>
                    </label>
                    {riders.length > 0 && (
                      <span className="text-[10px] text-slate-600 font-bold">
                        {riders.length} registered fleet riders
                      </span>
                    )}
                  </div>
                    ...
                    <div>
                      <select
                        onChange={(e) => {
                          ...
                          if (r) {
                            setMRiderName(r.name);
                            setMRiderPhone(r.phone);
                          }
                        }}
                        className="w-full px-3 py-1.5 rounded-xl bg-white border border-amber-300 text-slate-900 text-xs font-bold focus:outline-none"
                        defaultValue=""
                      >
                        <option value="" disabled>-- Quick Select Fleet Rider & Contact --</option>
                        {riders.map((r) => (
                          <option key={r.id} value={r.id}>
                            {r.name} ({r.phone}) • {r.vehicle || 'Tricycle'} [{r.status || 'Available'}]
                          </option>
                        ))}
                      </select>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    <div>
                      <label className="block text-slate-800 mb-1 font-bold">
                        Rider Full Name
                      </label>
                      <input
                        type="text"
                        placeholder="e.g. Kuya Jun / Kuya Arnel"
                        value={mRiderName}
                        onChange={(e) => setMRiderName(e.target.value)}
                        className="w-full px-3 py-1.5 rounded-xl bg-white border border-amber-300 text-slate-900 font-bold focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-800 mb-1 font-bold">
                      ...
                      <input
                        type="tel"
                        placeholder="e.g. 09625415941"
                        value={mRiderPhone}
                        onChange={(e) => setMRiderPhone(e.target.value)}
                        className="w-full px-3 py-1.5 rounded-xl bg-white border border-amber-300 text-slate-900 font-bold focus:outline-none"
                      />
                    </div>
                  </div>
                ...
                {filteredOrders.map((order) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Assigned Rider & Contact Details Card */}
                      <div className="mt-3 p-3 rounded-2xl bg-amber-50/70 border border-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-2xs">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-[#FED74C] text-[#111827] flex items-center justify-center font-black shrink-0 shadow-2xs">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {order.riderName ? (
                                <span className="font-heading font-black text-xs text-[#111827]">
                                  {order.riderName}
                                </span>
                              ) : (
                            ...
                            {order.riderPhone ? (
                              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                <span className="text-[11px] text-slate-600 font-semibold">Contact:</span>
                                <a
                                  href={`tel:${order.riderPhone}`}
                                  className="inline-flex items-center gap-1 text-[11px] font-black text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-md hover:bg-emerald-200 transition-colors"
                                  title="Call rider"
                                >
                                ...
                                <a
                                  ...
                                  className="inline-flex items-center gap-1 text-[10px] font-bold text-slate-600 bg-slate-200/80 px-1.5 py-0.5 rounded hover:bg-slate-300 transition-colors"
                              ...
                              <p className="text-[10px] text-slate-500 font-medium mt-0.5">
                                Add rider name & phone number to dispatch and show on customer tracking.
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => handleOpenRiderAssign(order)}
                          className="px-3 py-1.5 rounded-xl bg-white hover:bg-amber-100 text-[#111827] border border-amber-300 font-black text-xs flex items-center gap-1.5 shadow-2xs cursor-pointer active:scale-95 transition-all shrink-0 self-stretch sm:self-auto justify-center"
                        >
                          <User className="w-3.5 h-3.5 text-amber-800" />
                          <span>{order.riderName ? 'Change Rider' : '+ Assign Rider & Contact'}</span>
                        </button>
                      </div>
            ...
            {/* TAB 3: RIDERS & FLEET DIRECTORY */}
            {activeTab === 'riders' && (
              <div className="space-y-6">
                {/* Intro summary banner */}
                <div className="bg-white p-4 sm:p-5 rounded-3xl border border-amber-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-[#FED74C] text-[#111827] flex items-center justify-center font-black shrink-0 shadow-xs">
                      <Bike className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="font-heading font-black text-lg text-[#111827] flex items-center gap-2">
                        <span>Delivery Fleet & Riders Directory</span>
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-black bg-amber-100 text-amber-900 border border-amber-300">
                          {riders.length} Riders Registered
                        </span>
                      </h3>
                      <p className="text-xs text-slate-500 font-medium">
                        Add and manage delivery rider names, contact details, vehicle info, and dispatch live ice orders across Muñoz and San Jose City.
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={handleOpenAddRider}
                    className="px-4 py-2.5 rounded-2xl bg-[#111827] hover:bg-black text-[#FDD023] font-black text-xs uppercase tracking-wider flex items-center gap-2 shadow-md cursor-pointer transition-all active:scale-95 shrink-0 self-stretch sm:self-auto justify-center"
                  >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                  <div className="bg-white p-4 rounded-2xl border border-amber-200/90 shadow-xs">
                    <span className="text-[10px] uppercase font-black text-slate-500 tracking-wider block mb-1">
                    ...
                    <div className="flex items-baseline justify-between">
                      ...
                      <span className="text-[11px] font-bold text-slate-500">Riders</span>
                    </div>
                  </div>

                  <div className="bg-white p-4 rounded-2xl border border-amber-200/90 shadow-xs">
                    <span className="text-[10px] uppercase font-black text-emerald-700 tracking-wider block mb-1">
                      Available On Duty
                    </span>
                    <div className="flex items-baseline justify-between">
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-amber-200/90 shadow-xs">
                    ...
                    <div className="flex items-baseline justify-between">
                      <span className="font-heading font-black text-2xl text-amber-800">
                        {
                          riders.filter(
                            (r) =>
                              ...
                              orders.some(
                                (o) =>
                                  ...
                                  o.riderName?.toLowerCase() === r.name.toLowerCase()
                              )
                          ).length
                        }
                      </span>
                      <span className="text-[11px] font-bold text-amber-700">In Transit</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-amber-200/90 shadow-xs">
                    ...
                    <div className="flex items-baseline justify-between">
                      <span className="font-heading font-black text-2xl text-slate-600">
                        {riders.filter((r) => r.status === 'Off Duty').length}
                      </span>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-amber-200">
                  <div className="relative w-full sm:w-80">
                    <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
                    <input
                      type="text"
                      placeholder="Search rider name, phone, or vehicle..."
                      value={riderSearchQuery}
                      onChange={(e) => setRiderSearchQuery(e.target.value)}
                      className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:outline-none focus:border-[#111827]"
                    />
                  </div>

                  <div className="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto">
                    {['all', 'Available', 'On Delivery', 'Off Duty'].map((st) => (
                      <button
                        key={st}
                        type="button"
                ...
                {(() => {
                  const filteredRiders = riders.filter((r) => {
                    const matchesSearch =
                      ...
                      (r.vehicle && r.vehicle.toLowerCase().includes(riderSearchQuery.toLowerCase())) ||
                      (r.plateNumber && r.plateNumber.toLowerCase().includes(riderSearchQuery.toLowerCase()));

                    const matchesFilter =
                      riderStatusFilter === 'all' || r.status === riderStatusFilter;
                  ...
                  if (filteredRiders.length === 0) {
                    return (
                      <div className="bg-white rounded-3xl p-10 text-center border-2 border-dashed border-amber-200">
                        <div className="w-14 h-14 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center mx-auto mb-3">
                          <Bike className="w-7 h-7" />
                        </div>
                        <h4 className="font-heading font-black text-slate-800 text-base mb-1">
                          No Delivery Riders Found
                        </h4>
                        <p className="text-xs text-slate-500 mb-4">
                          {riderSearchQuery
                            ? `No riders match "${riderSearchQuery}"`
                            : 'No riders registered in this status category.'}
                        </p>
                        <button
                          type="button"
                          onClick={handleOpenAddRider}
                          className="px-4 py-2 rounded-xl bg-[#111827] text-[#FDD023] font-bold text-xs uppercase cursor-pointer"
                        >
                          + Add New Rider
                        </button>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {filteredRiders.map((rider) => {
                        const activeDeliveries = orders.filter(
                          (o) =>
                            o.riderName?.toLowerCase() === rider.name.toLowerCase() &&
                            o.status !== 'Delivered' &&
                            o.status !== 'Cancelled'
                        );
                        ...
                        return (
                          <div
                            key={rider.id}
                            className="bg-white rounded-3xl border-2 border-amber-200/90 hover:border-amber-400 p-5 shadow-sm flex flex-col justify-between transition-all"
                          >
                            <div className="space-y-4">
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-3">
                                  <div className="relative">
                                    <div className="w-12 h-12 rounded-2xl bg-[#FED74C] text-[#111827] flex items-center justify-center font-heading font-black text-lg shadow-xs">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-0.5 font-medium">
                                      ...
                                      <span className="truncate max-w-[150px]">
                                        {rider.vehicle || 'Insulated Ice Box Tricycle'}
                                      </span>
                                    </div>
                                    {rider.plateNumber && (
                                      <span className="inline-block mt-0.5 px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-100 text-slate-700">
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    handleToggleRiderStatus(
                                      rider,
                                      e.target.value as DeliveryRider['status']
                                    )
                                  }
                                  className={`text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-xl border cursor-pointer focus:outline-none ${
                                    rider.status === 'Available'
                              ...
                              {/* Contact Details Box with Direct Call & SMS Actions */}
                              <div className="bg-amber-50/80 p-3.5 rounded-2xl border border-amber-200/80 space-y-2">
                                ...
                                <div className="flex items-center gap-2 pt-1">
                                  <a
                                    ...
                                    className="flex-1 py-1.5 px-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[11px] flex items-center justify-center gap-1.5 transition-colors shadow-2xs"
                                  ...
                                  <a
                                    ...
                                    className="flex-1 py-1.5 px-2.5 rounded-xl bg-white hover:bg-amber-100 text-[#111827] border border-amber-300 font-bold text-[11px] flex items-center justify-center gap-1.5 transition-colors shadow-2xs"
                                    title={`Send SMS text to ${rider.name}`}
                                  >
                              ...
                              <div className="text-xs">
                                ...
                                {activeDeliveries.length > 0 ? (
                                  <div className="space-y-1.5">
                                    {activeDeliveries.map((act) => (
                                      <div
                                        key={act.id}
                                        className="p-2 rounded-xl bg-amber-100/60 border border-amber-300 text-[11px] flex items-center justify-between"
                                      >
                                        <div>
                                          <span className="font-mono font-bold text-amber-900">
                                            #{act.orderNumber}
                                          </span>{' '}
                                          • {act.customerName} ({act.bagSize || '5kg'} {act.productName})
                                        </div>
                                  ...
                                  <p className="text-[11px] text-slate-500 italic">
                                    No active deliveries right now (Ready for dispatch)
                                  </p>
                                )}
                              </div>
                              ...
                              {rider.notes && (
                                <p className="text-[11px] text-slate-600 bg-slate-50 p-2 rounded-xl border border-slate-200">
                                  <strong>Route / Notes:</strong> {rider.notes}
                                </p>
                              )}
                            </div>
                            ...
                            <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                              <button
                                type="button"
                                onClick={() => setAssignToOrderRider(rider)}
                                className="px-3 py-1.5 rounded-xl bg-[#111827] hover:bg-black text-[#FDD023] font-black text-xs uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95 transition-all"
                              >
                                <Truck className="w-3.5 h-3.5" />
                              ...
                              <div className="flex items-center gap-1.5">
                                <button
                                  ...
                                  className="p-2 rounded-xl bg-slate-100 hover:bg-amber-100 text-slate-700 hover:text-black border border-slate-200 transition-colors cursor-pointer"
                                  title="Edit rider contact & vehicle details"
                                >
        ...
        {showAddRiderModal && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between pb-3 border-b border-amber-200">
                <div className="flex items-center gap-2.5">
                  <div className="w-10 h-10 rounded-2xl bg-[#FED74C] text-[#111827] flex items-center justify-center font-black shadow-xs">
                    <Bike className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-heading font-black text-lg text-[#111827]">
                      {editingRider ? 'Edit Delivery Rider Contact' : 'Add New Delivery Rider'}
                    </h3>
                    <p className="text-xs text-slate-500 font-medium">
                      {editingRider
                        ? `Update contact and vehicle details for ${editingRider.name}`
                        : 'Register rider name and contact details to dispatch fleet'}
                    </p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleSaveRiderForm} className="space-y-4 pt-4 text-xs">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    placeholder="e.g. Kuya Juan Dela Cruz / Arnel Reyes"
                    value={riderFormName}
                    onChange={(e) => setRiderFormName(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-bold focus:outline-none focus:border-[#111827]"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    required
                    type="tel"
                    placeholder="e.g. 09625415941 / 0917-555-8812"
                    value={riderFormPhone}
                    onChange={(e) => setRiderFormPhone(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-bold focus:outline-none focus:border-[#111827]"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">
                    Used for direct customer updates, rider calls, and printed delivery slips.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-semibold focus:outline-none"
                    >
                      <option value="Insulated Ice Box Tricycle">Insulated Ice Box Tricycle</option>
                      <option value="Motorcycle with Cold Chest">Motorcycle with Cold Chest</option>
                      <option value="Express Delivery Van / Multicab">Express Delivery Van / Multicab</option>
                      <option value="Electric Cargo Bike with Cold Bag">Electric Cargo Bike with Cold Bag</option>
                      <option value="Pickup Truck / Hauler">Pickup Truck / Hauler</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-slate-700 font-bold mb-1">
                      Plate # or Fleet Code (Optional)
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. TR-3121 / YLO-01"
                      value={riderFormPlate}
                      onChange={(e) => setRiderFormPlate(e.target.value)}
                      className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-bold focus:outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setRiderFormStatus(e.target.value as any)}
                      className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-bold focus:outline-none"
                    >
                  ...
                  <div>
                    <label className="block text-slate-700 font-bold mb-1">
                      Service Route / Assigned Area (Optional)
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. Science City of Muñoz / San Jose"
                      value={riderFormNotes}
                      onChange={(e) => setRiderFormNotes(e.target.value)}
                      className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-medium focus:outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-100">
                  ...
                  <button
                    type="submit"
                    className="px-6 py-2.5 rounded-xl bg-[#111827] hover:bg-black text-[#FDD023] font-black uppercase tracking-wider shadow-sm cursor-pointer flex items-center gap-1.5"
                  >
                    <Save className="w-4 h-4" />
                    <span>{editingRider ? 'Save Changes' : 'Save & Register Rider'}</span>
                  </button>
                </div>
              </form>
            </div>
        ...
        {/* QUICK ASSIGN RIDER TO ORDER MODAL */}
        {assignToOrderRider && (
          <div
            className="fixed inset-0 z-70 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs overflow-y-auto"
            onClick={() => setAssignToOrderRider(null)}
          >
            <div
              className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border-2 border-amber-400 text-slate-900 my-4 space-y-4"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between pb-3 border-b border-amber-200">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="font-heading font-black text-base text-[#111827]">
                      Assign Order to {assignToOrderRider.name}
                    </h4>
                    <p className="text-xs text-slate-500 font-medium">
                      📞 {assignToOrderRider.phone} • {assignToOrderRider.vehicle || 'Tricycle'}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="text-xs space-y-2">
                ...
                {orders.filter((o) => o.status === 'Pending' || o.status === 'Preparing').length === 0 ? (
                  <div className="p-6 text-center bg-slate-50 rounded-2xl border border-slate-200">
                    <p className="font-bold text-slate-700">No pending orders awaiting dispatch!</p>
                    <p className="text-xs text-slate-500 mt-1">All current orders already have assigned riders or are delivered.</p>
                  </div>
                ) : (
                  <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                    {orders
                      ...
                      .map((ord) => (
                        <div
                          key={ord.id}
                          className="p-3 rounded-2xl border border-amber-200 bg-amber-50/50 hover:bg-amber-100/70 transition-colors flex items-center justify-between gap-3"
                        >
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                            ...
                            <p className="text-[11px] text-slate-600 truncate mt-0.5">
                              {ord.quantity}x {ord.bagSize || '5kg'} {ord.productName} • {ord.deliveryAddress} ({ord.cityArea || 'Muñoz'})
                            </p>
                            <p className="text-[10px] text-amber-800 font-bold mt-0.5">
                              COD: ₱{ord.total.toLocaleString()} • Status: {ord.status}
                            </p>
                          </div>

                          <button
                            type="button"
                            onClick={() => handleAssignRiderToOrderQuick(assignToOrderRider, ord)}
                            className="px-3 py-1.5 rounded-xl bg-[#111827] hover:bg-black text-[#FDD023] font-black text-xs uppercase tracking-wider shrink-0 cursor-pointer shadow-xs active:scale-95"
                          >
                            Dispatch
                          </button>
                        </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-2 text-xs">
                ...
                {/* Assigned Rider Information on Slip */}
                <div className="border-t border-slate-200 pt-2 flex justify-between items-center bg-amber-50/70 p-2.5 rounded-xl">
                  <div>
                    <span className="text-[10px] font-black text-slate-500 uppercase block">Assigned Rider:</span>
                    <span className="font-black text-slate-900 text-xs block">
                      {selectedOrderForReceipt.riderName || 'Unassigned'}
                    </span>
                  ...
                  <button
                    type="button"
                    onClick={() => handleOpenRiderAssign(selectedOrderForReceipt)}
                    className="px-2.5 py-1 rounded-lg bg-white border border-amber-300 text-[11px] font-black text-[#111827] hover:bg-amber-100 cursor-pointer shadow-2xs"
                  >
                    {selectedOrderForReceipt.riderName ? 'Change Rider' : '+ Assign Rider'}
                  </button>
                </div>
        ...
        {orderForRiderAssign && (
          <div
            ...
            onClick={() => !isSavingRider && setOrderForRiderAssign(null)}
          >
            <div
              className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border-2 border-amber-400 text-slate-900 space-y-4"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between pb-3 border-b border-amber-200">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">
                      Order #{orderForRiderAssign.orderNumber} • {orderForRiderAssign.customerName}
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setOrderForRiderAssign(null)}
                  className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
                ...
                <div className="bg-amber-50/70 p-3 rounded-2xl border border-amber-200">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold text-slate-500 hover:text-black underline cursor-pointer"
                    >
                      Clear / New
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {savedRiders.map((r, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => {
                          setAssignRiderName(r.name);
                          setAssignRiderPhone(r.phone);
                          if (r.vehicle) setAssignRiderNotes(r.vehicle);
                        }}
                        className={`text-[11px] font-bold px-2.5 py-1 rounded-xl border transition-all cursor-pointer flex items-center gap-1 ${
                          assignRiderName === r.name
                            ? 'bg-[#111827] text-[#FED74C] border-[#111827]'
                            : 'bg-white hover:bg-amber-100 text-slate-800 border-amber-300'
                        }`}
                      >
                        <UserCheck className="w-3 h-3 text-amber-600" />
                        <span>{r.name}</span>
                        <span className="text-[10px] opacity-75 font-mono">({r.phone.slice(-4)})</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-3 text-xs">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. Kuya Jun / Mark Santos"
                    value={assignRiderName}
                    onChange={(e) => setAssignRiderName(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-bold focus:outline-none focus:border-[#111827]"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    placeholder="e.g. 09625415941"
                    value={assignRiderPhone}
                    onChange={(e) => setAssignRiderPhone(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-bold focus:outline-none focus:border-[#111827]"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">
                    This phone number will be displayed to customer on the live tracker and included on the printed slip.
                  </p>
                </div>

                <div>
                  <label className="block text-slate-700 font-bold mb-1">
                    Vehicle / Delivery Notes (Optional)
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. Insulated Ice Box Tricycle / Plate #1234"
                    value={assignRiderNotes}
                    onChange={(e) => setAssignRiderNotes(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-medium focus:outline-none focus:border-[#111827]"
                  />
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-end gap-2">
                <button
                  ...
                  onClick={() => setOrderForRiderAssign(null)}
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs cursor-pointer"
                >
                  Cancel
                </button>
                ...
                <button
                  type="button"
                  disabled={isSavingRider}
                  onClick={() => handleSaveRider(false)}
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-black text-white font-bold text-xs cursor-pointer"
                >
                  Save Rider
                </button>
                ...
                <button
                  type="button"
                  disabled={isSavingRider}
                  onClick={() => handleSaveRider(true)}
                  className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#FED74C] hover:bg-amber-400 text-[#111827] font-black text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                >
                  <Truck className="w-3.5 h-3.5" />
